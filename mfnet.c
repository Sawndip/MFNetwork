...
#define w_ie 0.1e-3 //0.2e-3 //0.1e-3 //0.2e-3 /* weights */
#define w_ei 0.6e-3 //0.4e-3 //0.6e-3  
#define w_ii 0.6e-3 //0.8e-3 //0.6e-3 
#define w_min 0.0
#define w_len 1.0e-3
#define rho_init 0.1
#define cpre 0.56
...
#define sigma 0.5e-3 //5.e-3 //0.5e-3 /* noise */
...
int main(void) {
	...
	float nu_e[NintT], nu_i[NintT], w_ee[NintT], rho[wNintT];
	...
	for(it = 1; it < NintT; it++){
		...
		if (it % mfNintT == 0){
			...
			rho[jt] = updateWeight(rho[jt-1], dwt, nu_e[it-1], cpre, cpost);
			w_ee[it] = w_min + (w_len * rho[jt]);
		}
		else{
			w_ee[it] = w_ee[it-1];
		}
